...
VL NOISE (2026-07-02, Complementary Modification ONLY — не в патче, не в других паках; РАНТАЙМ-ПРОВЕРКА ЮЗЕРОМ PASS 2026-07-02): анимированный 3D-шум плотности ломает однородный конус на дрейфующие клубы. irlite_vlNoise(worldP, w2) = 2-октавный fBm на паковском Noise3D (псевдо-3D из noisetex-слайсов, commonFunctions.glsl — в скоупе deferred2 через common.glsl). Модулирует ТОЛЬКО inscatter (Beer-Lambert не тронут): в марше после расчёта inscatter, гейт if(inscatter>1e-5) (пропуск мёртвых/затенённых шагов), inscatter *= mix(1.0, 2.0*noiseVal, IRLITE_VL_NOISE_AMOUNT); mean(fBm)~0.5 -> 2*noise сохраняет среднюю яркость. Позиция = startWorld+pos (абсолютный мир, клубы не плывут с камерой). Дефайны: IRLITE_VL_NOISE (toggle, ON), _AMOUNT 0.6, _SCALE 2.0 (блоков на texel базовой октавы; период поля = 128*SCALE), _SPEED 1.0, _STRIDE 1 [1-4] (тап шума каждый N-й шаг, между — кэш noiseVal, init 0.5=нейтраль; зеркало паттерна IRLITE_VL_SHADOW_STRIDE; >1 без TAA может полосить вдоль луча); заведены в shaders.properties (экран IRLITE_VL_SETTINGS + sliders) и lang/en_US. Две ловушки, найденные верификацией и зашитые в реализацию: (1) frameTimeCounter wrap каждые 3600с -> ветер задан в ЦЕЛЫХ периодах поля за цикл: wind = frameTimeCounter*(SPEED/3600)*vec3(12,4,8), октава-2 получает wind*2; слайдер SPEED кратен 0.25 и векторы кратны 4 -> смещение к сбросу кратно периоду, шва нет (менять значения слайдера/векторы только с сохранением этого инварианта); (2) октава-2 (фича SCALE/2.53 блока) ниже Найквиста при шаге марша до ~9.6 блока -> per-light вес noiseW2 = 0.35*clamp(SCALE/(2.53*stepLen),0,1), веса октав нормированы (1-w2, w2) -> mean сохранён.
VL-NOISE СТАТУС ПАУЗЫ (2026-07-02, юзер переключился на другой проект): фича done+PASS на Complementary; ОСТАЛОСЬ: (а) порт в остальные 5 паков (Photon/IterationRP/BSL/Solas/RethinkingVoxels/Bliss — Noise3D пак-специфичен, адаптировать источник шума: у Photon свои blue-noise/3D-текстуры, у BSL-линейки Noise3D аналогичен); (б) patches/complementaryreimagined.irlights НЕ регенерирован — noise живёт ТОЛЬКО в Shadres/Modification (gitignored!) + run-паке; РИСК cookie-стайл потери при регене патча из отставшей копии — при возврате к проекту сначала реген патча по команде юзера; (в) попутный страйд-фикс vec4 cookie в структуре Complementary Modification (только поле, без сэмплинга — см. project-spotlight-gobo-cookie-plan).
